function brain_blocking_group(names, GeometryPosts, GeometryPres, ROIAnalyzes, configs, Eposts, EpreFits, Epres)
for k = 1:length(names)
    name = names(k);
    GeometryPost = GeometryPosts(k);
    GeometryPre = GeometryPres(k);
    ROIAnalyze = ROIAnalyzes(k);
    config = configs(k);
    Epost = Eposts(k);
    EpreFit = EpreFits(k);
    Epre = Epres(k);
    ...
    %datetimeRun = strrep(datestr(datetime('now')), {' ', '-', ':'}, '_');

    titleRun = append('BBG_', name, datetimeRun);
    mkdir(titleRun);
    ...
    elect_locs = cell2mat(config(1)); %Finding distances from first (Front) electrode
    for i = 1:length(ROI_centers)
    ...
    allCylindersDiff(k) = cylindersDiff;
    allCylindersPre(k) = cylindersPre;
    allCylindersPost(k) = cylindersPost;
    allClosestElectIdx(k) = closestElectIdx;
    allClosestDist2Elect(k) = closestDist2Elect;
...
field = [];
for k = 1:length(names)
    cylindersDiff = allCylindersDiff(k);
    cylindersPre = allCylindersPre(k);
    cylindersPost = allCylindersPost(k);
    closestElectIdx = allClosestElectIdx(k);
    ...
    for j = 1:length(ROI_centers)
        ...
        if closestElectIdx(j) == 1
            scatter(cylindersDiff(closestElectIdx(j),j).conds, cylindersDiff(closestElectIdx(j),j).fieldMag, 'DisplayName','Front Electrode ' + names(k), MarkerEdgeColor='r')
        else
