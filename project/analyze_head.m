 function [EAngle, EMagnitude, ROI, Segmentation] = analyze_head(electrodes, ROIcenter, geometrySTR, pre, post, mask_prepI, mask_prepF, ROI_radii, name)
...
if(isempty(electrodes))
    ...
    for fn = fieldnames(Segmentation.diff)'
        Segmentation.voxChangeSum.(fn{1}) = sum(Segmentation.diff.(fn{1}), "all");
    end
else
    %% Full run:
